...
export function WorldMap({ 
  dots = [], 
  lineColor = "#34d399", // Using emerald-400 as default instead of blue
  showLabels = true,
  animationDuration = 2,
  loop = true
}: MapProps) {
  ...
  const svgMap = useMemo(
    () => map.getSVG({
      radius: 0.22,
      color: "#ffffff40", // Light white dots for dark mode
      shape: "circle",
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-[#111111] text-emerald-400 border border-white/10 shadow-[0_0_20px_rgba(52,211,153,0.1)]">
                          {dot.start.label}
                        </span>
                      </div>
                    </foreignObject>
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 bg-[#111111] text-emerald-400 px-3 py-2 rounded-lg text-sm font-bold border border-white/10 sm:hidden shadow-[0_0_30px_rgba(52,211,153,0.1)]"
          >
            {hoveredLocation}
          </motion.div>
        )}
